...
{
    delete [] this->route;
    this->route = (other.route_arraysize==0) ? nullptr : new ::omnetpp::opp_string[other.route_arraysize];
...
{
    ::omnetpp::opp_string *route2 = (size==0) ? nullptr : new ::omnetpp::opp_string[size];
...
const char * Packet::getRoute(unsigned int k) const
{
    if (k>=route_arraysize) throw omnetpp::cRuntimeError("Array of size %d indexed by %d", route_arraysize, k);
    return this->route[k].c_str();
}

void Packet::setRoute(unsigned int k, const char * route)
{
    if (k>=route_arraysize) throw omnetpp::cRuntimeError("Array of size %d indexed by %d", route_arraysize, k);
    this->route[k] = route;
}
